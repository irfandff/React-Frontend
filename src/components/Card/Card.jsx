...
export default function Card({product}) {
  return (
    <Link to={`/products/${product.slug}` ?? ''} className="flex flex-col max-w-[370px] flex-wrap p-[16px] bg-[#003B46] hover:ring-opacity-40 active:ring-5 active:ring-[#C4DAD2] hover:ring-4 active:ring-2 active:ring-opacity-90" >
    <div className="flex flex-col max-w-[370px] flex-wrap p-[16px] bg-[#003B46]" >
          <img src={product.imageUrl ?? ''} alt={product.name ?? 'No name'} className="block max-h-[300px] mb-4 object-cover" />
            <div className="flex flex-col gap-2">
                ...
                <div>
                {product.stock <= 0 ? (
                    <p className="text-xl font-semibold text-center text-red-500 mt-4">Out of Stock</p>
                  ) : (product.stock <= 25 && product.stock !== 0) ? (
                    <>
                       <Button type="button" className="inline-flex items-center justify-center gap-2 p-4 bg-[#F5F5F5] text-center hover:bg-[#C4DAD2] text-black active:bg-[#003B46]">
